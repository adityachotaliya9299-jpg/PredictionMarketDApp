...
export default function USDCMarketDetailPage() {
  ...
  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"24px 16px 48px", boxSizing:"border-box" as const }}>
      ...
      <Link href="/usdc" style={{ display:"inline-flex", alignItems:"center", gap:6, color:"var(--faint)", textDecoration:"none", fontSize:14, marginBottom:20 }}>
      ...
      <div style={{ ...card, marginBottom:20 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12, flexWrap:"wrap" as const }}>
          <span style={{ padding:"4px 10px", borderRadius:99, fontSize:11, fontWeight:700, background:"rgba(var(--up-rgb),0.15)", color:"var(--up)", border:"1px solid rgba(var(--up-rgb),0.3)" }}>💵 USDC</span>
          <span style={{ padding:"4px 10px", borderRadius:99, fontSize:11, fontWeight:700, background:isOpen?"rgba(var(--up-rgb),0.1)":"rgba(var(--warn-rgb),0.1)", color:isOpen?"var(--up)":"var(--warn)", border:`1px solid ${isOpen?"rgba(var(--up-rgb),0.3)":"rgba(var(--warn-rgb),0.3)"}` }}>
            {market.resolved?"RESOLVED":expired?"EXPIRED":"LIVE"}
          </span>
        </div>
        <h1 style={{ color:"var(--text)", fontSize:"clamp(16px,3vw,22px)", fontWeight:800, lineHeight:1.3, marginBottom:14 }}>{market.question}</h1>
        <div style={{ display:"flex", flexWrap:"wrap" as const, gap:16, alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            <DollarSign size={14} color="var(--up)"/>
            <span style={{ color:"var(--text)", fontWeight:700, fontFamily:"var(--font-mono)" }}>{formatUnits(market.totalPool, 6)} USDC</span>
            <span style={{ color:"var(--faint)", fontSize:13 }}>pool</span>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            <Clock size={14} color="var(--warn)"/>
            <span style={{ color:"var(--muted)", fontSize:13 }}>{formatTimeLeft(market.expirationTime)}</span>
          </div>
      ...
      <div className="mob-panel" style={{ marginBottom:16 }}>
        {isOpen&&isConnected&&(
          <div style={card}>
            <h3 style={{ color:"var(--text)", fontWeight:700, fontSize:15, marginBottom:16 }}>Trade with USDC</h3>
            <p style={{ color:"var(--faint)", fontSize:12, marginBottom:12 }}>Balance: {usdcBalance.formatted} USDC</p>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
              {(["YES","NO"] as const).map(s=>(
                <button key={s} onClick={()=>setSide(s)} style={{ padding:"11px 0", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer", border:side===s?`1px solid ${s==="YES"?"rgba(var(--up-rgb),0.5)":"rgba(var(--down-rgb),0.5)"}`:"1px solid rgba(var(--fg-rgb),0.08)", background:side===s?(s==="YES"?"rgba(var(--up-rgb),0.15)":"rgba(var(--down-rgb),0.15)"):"transparent", color:side===s?(s==="YES"?"var(--up)":"var(--down)"):"var(--faint)" }}>
            ...
            <div style={{ position:"relative", marginBottom:12 }}>
              ...
              <span style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", color:"var(--up)", fontSize:13, fontWeight:600 }}>USDC</span>
            </div>
            ...
            {needsApproval?(
              <button onClick={async()=>{try{await approve(amount);setTimeout(()=>refetchAllowance(),2000);}catch(e:any){setError(e?.shortMessage||"Failed");}}}
                ...
                style={{ width:"100%", padding:12, borderRadius:10, background:"linear-gradient(135deg,var(--warn),#f97316)", border:"none", color:"var(--on-accent)", fontWeight:700, cursor:"pointer" }}>
      ...
      <div className="detail-grid">
        ...
        <div>
          <div style={card}>
            <h3 style={{ color:"var(--text)", fontWeight:700, marginBottom:14, fontSize:14 }}>Probability</h3>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
              <span style={{ color:"var(--up)", fontWeight:700, fontFamily:"var(--font-mono)" }}>YES {market.yesPct}%</span>
              <span style={{ color:"var(--down)", fontWeight:700, fontFamily:"var(--font-mono)" }}>NO {market.noPct}%</span>
            </div>
            <div style={{ height:10, borderRadius:99, background:"rgba(var(--down-rgb),0.15)", overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${market.yesPct}%`, background:"linear-gradient(90deg,var(--up),var(--accent))", borderRadius:99, transition:"width 0.5s" }}/>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginTop:16 }}>
              <div style={{ padding:14, borderRadius:12, background:"rgba(var(--up-rgb),0.1)", border:"1px solid rgba(var(--up-rgb),0.2)", textAlign:"center" }}>
                <div style={{ fontSize:24, fontWeight:900, fontFamily:"var(--font-mono)", color:"var(--up)" }}>{market.yesPct}%</div>
                <div style={{ color:"var(--faint)", fontSize:12, marginTop:4 }}>YES Pool: {formatUnits(market.yesPool,6)} USDC</div>
              </div>
              <div style={{ padding:14, borderRadius:12, background:"rgba(var(--down-rgb),0.1)", border:"1px solid rgba(var(--down-rgb),0.2)", textAlign:"center" }}>
                <div style={{ fontSize:24, fontWeight:900, fontFamily:"var(--font-mono)", color:"var(--down)" }}>{market.noPct}%</div>
                <div style={{ color:"var(--faint)", fontSize:12, marginTop:4 }}>NO Pool: {formatUnits(market.noPool,6)} USDC</div>
              </div>
            </div>
          </div>
            ...
            <div style={card}>
              ...
              {market.userYesShares > BigInt(0) && (
                <div style={{ display:"flex", justifyContent:"space-between", padding:"4px 0" }}>
                  ...
                  <span style={{ color:"var(--up)", fontFamily:"var(--font-mono)" }}>{formatUnits(market.userYesShares,6)} USDC</span>
                </div>
              )}
              {market.userNoShares > BigInt(0) && (
                <div style={{ display:"flex", justifyContent:"space-between", padding:"4px 0" }}>
                  <span style={{ color:"var(--muted)", fontSize:13 }}>NO Shares</span>
                  <span style={{ color:"var(--down)", fontFamily:"var(--font-mono)" }}>{formatUnits(market.userNoShares,6)} USDC</span>
                </div>
              )}
            </div>
          )}
          ...
          {market.resolved && !market.hasClaimed && (market.userYesShares > BigInt(0) || market.userNoShares > BigInt(0)) && (
            <div style={{ ...card, background:"rgba(var(--up-rgb),0.08)", border:"1px solid rgba(var(--up-rgb),0.25)" }}>
              <p style={{ color:"var(--text)", fontWeight:700, marginBottom:12 }}>🏆 Claim Your USDC Reward</p>
              {claimSuccess?(
        ...
        <div className="desk-panel" style={{ position:"sticky", top:80 }}>
          {isOpen && isConnected ? (
            <div style={card}>
              <h3 style={{ color:"var(--text)", fontWeight:700, fontSize:15, marginBottom:16 }}>Trade with USDC</h3>
              <p style={{ color:"var(--faint)", fontSize:12, marginBottom:12 }}>Balance: <span style={{ color:"var(--up)", fontWeight:600 }}>{usdcBalance.formatted} USDC</span></p>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
                {(["YES","NO"] as const).map(s=>(
                  <button key={s} onClick={()=>setSide(s)} style={{ padding:"11px 0", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer", border:side===s?`1px solid ${s==="YES"?"rgba(var(--up-rgb),0.5)":"rgba(var(--down-rgb),0.5)"}`:"1px solid rgba(var(--fg-rgb),0.08)", background:side===s?(s==="YES"?"rgba(var(--up-rgb),0.15)":"rgba(var(--down-rgb),0.15)"):"transparent", color:side===s?(s==="YES"?"var(--up)":"var(--down)"):"var(--faint)" }}>
                    {s==="YES"?"▲":"▼"} {s} {s==="YES"?market.yesPct:market.noPct}%
              ...
              <div style={{ display:"flex", gap:6, marginBottom:12 }}>
                {["1","5","10","50"].map(q=>(
                  <button key={q} onClick={()=>setAmount(q)} style={{ flex:1, padding:"5px 0", borderRadius:8, fontSize:11, fontFamily:"var(--font-mono)", cursor:"pointer", border:amount===q?"1px solid rgba(var(--up-rgb),0.4)":"1px solid rgba(var(--fg-rgb),0.06)", background:amount===q?"rgba(var(--up-rgb),0.1)":"transparent", color:amount===q?"var(--up)":"var(--faint)" }}>{q}</button>
                ))}
              </div>
              ...
              {needsApproval?(
                <button onClick={async()=>{try{await approve(amount);setTimeout(()=>refetchAllowance(),2000);}catch(e:any){setError(e?.shortMessage||"Failed");}}}
                  disabled={approvePending||approveConfirming}
                  style={{ width:"100%", padding:13, borderRadius:12, background:"linear-gradient(135deg,var(--warn),#f97316)", border:"none", color:"var(--on-accent)", fontWeight:700, fontSize:14, cursor:"pointer" }}>
                  {approvePending||approveConfirming?"⏳ Approving...":"✅ Approve USDC"}
                </button>
              ):(
                <button onClick={handleTrade} disabled={isPending||isConfirming||!amount}
              ...
              <p style={{ color:"var(--faint-2)", fontSize:11, textAlign:"center", marginTop:8 }}>Need Sepolia USDC? Get from Circle faucet</p>
            </div>
          ) : !isConnected ? (
            <div style={{ ...card, textAlign:"center" }}>
              <p style={{ color:"var(--faint)", fontSize:14 }}>Connect wallet to trade</p>
            </div>
          ) : (
            <div style={{ ...card, textAlign:"center" }}>
              <p style={{ color:"var(--faint)", fontSize:14 }}>Market is {market.resolved?"resolved":"expired"}</p>
            </div>
          )}
        </div>
      </div>
