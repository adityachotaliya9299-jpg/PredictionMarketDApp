  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
        <span style={{ color:"#10b981", fontWeight:700, fontFamily:"monospace", fontSize:18 }}>YES {yes.toFixed(1)}%</span>
        <span style={{ color:"#ef4444", fontWeight:700, fontFamily:"monospace", fontSize:18 }}>NO {no.toFixed(1)}%</span>
      </div>
      <div style={{ height:12, borderRadius:99, background:"rgba(239,68,68,0.2)", overflow:"hidden", position:"relative" }}>
        <div style={{ height:"100%", width:`${yes}%`, background:"linear-gradient(90deg,#10b981,#22d3ee)", borderRadius:99, transition:"width 0.5s ease" }}/>
        <div style={{ position:"absolute", left:"50%", top:0, height:"100%", width:2, background:"rgba(255,255,255,0.2)" }}/>
      </div>
  ...
  return (
    <div style={{ display:"flex", gap:8 }}>
      <a href={twitterUrl} target="_blank" rel="noopener noreferrer"
        style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 14px", borderRadius:8, background:"rgba(29,161,242,0.15)", border:"1px solid rgba(29,161,242,0.3)", color:"#1da1f2", fontSize:13, fontWeight:600, textDecoration:"none" }}>
        <Twitter size={14}/>Share on X
      </a>
      <button onClick={copyLink}
        style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 14px", borderRadius:8, background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", color:"#9ca3af", fontSize:13, cursor:"pointer" }}>
...
function TradePanel({ marketAddress, probability, feeBps, onSuccess }: {
  ...
  if (!isConnected) return (
    <div style={{ padding:20, borderRadius:16, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", textAlign:"center" }}>
      <p style={{ color:"#6b7280", fontSize:14, marginBottom:12 }}>Connect wallet to trade</p>
      <Link href="/" style={{ color:"#22d3ee", fontSize:13 }}>Connect from navbar →</Link>
    </div>
  );
  ...
  if (success) return (
    <div style={{ padding:24, borderRadius:16, background:"rgba(16,185,129,0.1)", border:"1px solid rgba(16,185,129,0.3)", textAlign:"center" }}>
      ...
      <p style={{ color:"#10b981", fontWeight:700 }}>Trade Confirmed!</p>
      <p style={{ color:"#6b7280", fontSize:13, marginTop:4 }}>Your {side} shares purchased.</p>
    </div>
  );

  const quickAmts = ["0.01","0.05","0.1","0.5"];
  ...
  return (
    <div style={{ padding:20, borderRadius:16, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:16 }}>
        <button onClick={()=>setSide("YES")} style={{ padding:"12px 0", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer", border: side==="YES"?"1px solid rgba(16,185,129,0.5)":"1px solid rgba(255,255,255,0.08)", background: side==="YES"?"rgba(16,185,129,0.15)":"transparent", color: side==="YES"?"#10b981":"#6b7280" }}>
          ▲ YES — {yesPct.toFixed(1)}%
        </button>
        <button onClick={()=>setSide("NO")} style={{ padding:"12px 0", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer", border: side==="NO"?"1px solid rgba(239,68,68,0.5)":"1px solid rgba(255,255,255,0.08)", background: side==="NO"?"rgba(239,68,68,0.15)":"transparent", color: side==="NO"?"#ef4444":"#6b7280" }}>
          ▼ NO — {noPct.toFixed(1)}%
        </button>
      </div>
      ...
      <div style={{ marginBottom:12 }}>
        ...
        <div style={{ position:"relative" }}>
          <input type="number" value={amount} onChange={e=>{setAmount(e.target.value);setError("");}} placeholder="0.0" min="0" step="0.001"
            style={{ width:"100%", background:"rgba(0,0,0,0.3)", border:`1px solid ${error?"rgba(239,68,68,0.5)":"rgba(255,255,255,0.1)"}`, borderRadius:10, padding:"12px 48px 12px 14px", color:"white", fontSize:16, fontFamily:"monospace", outline:"none", boxSizing:"border-box" }}/>
          <span style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", color:"#6b7280", fontSize:13 }}>ETH</span>
        </div>
        <div style={{ display:"flex", gap:6, marginTop:8 }}>
          {quickAmts.map(q=>(
            <button key={q} onClick={()=>setAmount(q)} style={{ flex:1, padding:"6px 0", borderRadius:8, fontSize:12, fontFamily:"monospace", cursor:"pointer", border: amount===q?"1px solid rgba(34,211,238,0.4)":"1px solid rgba(255,255,255,0.06)", background: amount===q?"rgba(34,211,238,0.1)":"transparent", color: amount===q?"#22d3ee":"#6b7280" }}>{q}</button>
          ))}
        </div>
      </div>
      ...
      {amountWei > BigInt(0) && (
        <div style={{ background:"rgba(0,0,0,0.2)", borderRadius:10, padding:"10px 14px", marginBottom:12, fontSize:12, fontFamily:"monospace" }}>
          <div style={{ display:"flex", justifyContent:"space-between", color:"#6b7280", marginBottom:4 }}>
            ...
            <span style={{ color:"#ef4444" }}>-{formatETH(fee, 6)}</span>
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", color:"white", fontWeight:700, borderTop:"1px solid rgba(255,255,255,0.06)", paddingTop:6 }}>
            ...
            <span style={{ color: side==="YES"?"#10b981":"#ef4444" }}>{formatETH(net, 6)} {side}</span>
          </div>
        </div>
      )}
      ...
      <button onClick={handleTrade} disabled={isPending||isConfirming||!amount}
        style={{ width:"100%", padding:14, borderRadius:12, fontWeight:700, fontSize:15, cursor: isPending||isConfirming||!amount?"not-allowed":"pointer", opacity: !amount?0.5:1, border:"none", background: side==="YES"?"linear-gradient(135deg,#10b981,#22d3ee)":"linear-gradient(135deg,#ef4444,#f97316)", color: side==="YES"?"black":"white", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
        {isPending?"⏳ Confirm in wallet...":isConfirming?"⏳ Confirming...":`⚡ Buy ${side}${amount?` — ${amount} ETH`:""}`}
      </button>

      <p style={{ color:"#4b5563", fontSize:11, textAlign:"center", marginTop:10, lineHeight:1.5 }}>
        Parimutuel pool — winners split the entire pool proportionally
      </p>
    </div>
  );
}
  ...
  if (claimed || hasClaimed) return (
    <div style={{ padding:20, borderRadius:16, background:"rgba(16,185,129,0.1)", border:"1px solid rgba(16,185,129,0.3)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          <p style={{ color:"#10b981", fontWeight:700 }}>Reward Claimed!</p>
          <p style={{ color:"#6b7280", fontSize:12 }}>Winnings sent to your wallet.</p>
        </div>
      </div>
    </div>
  );
  ...
  if ((status === "resolved" || status === "invalid") && userWon && !hasClaimed) return (
    <div style={{ padding:20, borderRadius:16, background:"rgba(16,185,129,0.1)", border:"1px solid rgba(16,185,129,0.3)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
        ...
        <div>
          <p style={{ color:"white", fontWeight:700 }}>You Won!</p>
          <p style={{ color:"#6b7280", fontSize:12 }}>Outcome: {outcomeLabel(marketInfo.outcome)}</p>
        </div>
      </div>
      ...
      <button onClick={async()=>{try{await claimReward();setClaimed(true);onSuccess();}catch(e:any){setError(e?.shortMessage||"Failed");}}}
        ...
        style={{ width:"100%", padding:14, borderRadius:12, background:"linear-gradient(135deg,#10b981,#22d3ee)", border:"none", color:"black", fontWeight:700, fontSize:15, cursor:"pointer" }}>
...
export default function MarketDetailPage() {
  ...
  const label = { color:"#6b7280", fontSize:12, marginBottom:4 };
  const value = { color:"white", fontWeight:600, fontSize:14, fontFamily:"monospace" };
  ...
  const isResolved = status === "resolved" || status === "invalid";
  const showClaim = status === "expired" || isResolved;
  const hasPosition = yesShares > BigInt(0) || noShares > BigInt(0);

  const statusColors: Record<string, string> = {
    open:"#10b981", expired:"#fbbf24", resolved:"#3b82f6", paused:"#ef4444", invalid:"#6b7280"
  };

  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"40px 24px", boxSizing:"border-box" as const }}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}}`}</style>

      {/* Back */}
      <Link href="/" style={{ display:"inline-flex", alignItems:"center", gap:6, color:"#6b7280", textDecoration:"none", fontSize:14, marginBottom:24 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            {isResolved && (
              <span style={{ padding:"4px 10px", borderRadius:99, fontSize:11, fontWeight:700, background:"rgba(59,130,246,0.15)", color:"#3b82f6", border:"1px solid rgba(59,130,246,0.3)" }}>
                Outcome: {outcomeLabel(marketInfo.outcome)}
              </span>
            )}
          </div>
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <ShareButtons question={marketInfo.question} yes={yesPct}/>
            <a href={`https://sepolia.etherscan.io/address/${marketAddress}`} target="_blank" rel="noopener noreferrer"
              style={{ color:"#6b7280", display:"flex" }}><ExternalLink size={16}/></a>
          </div>
        </div>

        <h1 style={{ color:"white", fontSize:22, fontWeight:800, lineHeight:1.3, marginBottom:16 }}>
          {marketInfo.question}
        </h1>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:20 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            ...
            <span style={{ color:"#9ca3af", fontSize:13 }}>
              {isResolved ? `Resolved ${formatTimestamp(marketInfo.resolutionTime)}` : formatTimeLeft(marketInfo.expirationTime)}
            </span>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginTop:16 }}>
              <div style={{ padding:16, borderRadius:12, background:"rgba(16,185,129,0.1)", border:"1px solid rgba(16,185,129,0.25)", textAlign:"center" }}>
                <div style={{ fontSize:28, fontWeight:900, fontFamily:"monospace", color:"#10b981" }}>{yesPct.toFixed(1)}%</div>
                <div style={{ color:"#6b7280", fontSize:12, marginTop:4 }}>YES Probability</div>
              </div>
              <div style={{ padding:16, borderRadius:12, background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.25)", textAlign:"center" }}>
                <div style={{ fontSize:28, fontWeight:900, fontFamily:"monospace", color:"#ef4444" }}>{noPct.toFixed(1)}%</div>
          ...
          <div style={card}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {[
                ...
                <div key={r.label} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding: r.bold?"8px 0 0":"0", borderTop: r.bold?"1px solid rgba(255,255,255,0.06)":"none" }}>
