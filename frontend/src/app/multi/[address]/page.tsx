...
export default function MultiMarketPage() {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 16px 48px", boxSizing:"border-box" as const }}>
      ...
      <Link href="/multi" style={{ display:"inline-flex", alignItems:"center", gap:6, color:"var(--faint)", textDecoration:"none", fontSize:14, marginBottom:20 }}>
      ...
      <div style={{ ...card, marginBottom:20 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12, flexWrap:"wrap" as const }}>
          <span style={{ padding:"4px 10px", borderRadius:99, fontSize:11, fontWeight:700, background:"rgba(var(--accent3-rgb),0.15)", color:"var(--accent-3)", border:"1px solid rgba(var(--accent3-rgb),0.3)" }}>
            MULTI-OUTCOME
          </span>
          <span style={{ padding:"4px 10px", borderRadius:99, fontSize:11, fontWeight:700,
            background: isOpen?"rgba(var(--up-rgb),0.1)":expired&&!market.resolved?"rgba(var(--warn-rgb),0.1)":"rgba(var(--accent2-rgb),0.1)",
            color: isOpen?"var(--up)":expired&&!market.resolved?"var(--warn)":"var(--accent-2)",
            border: `1px solid ${isOpen?"rgba(var(--up-rgb),0.3)":expired&&!market.resolved?"rgba(var(--warn-rgb),0.3)":"rgba(var(--accent2-rgb),0.3)"}` }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap" as const, gap:16, alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            ...
            <span style={{ color:"var(--text)", fontWeight:700, fontFamily:"var(--font-mono)" }}>{formatETH(totalPool)}</span>
            <span style={{ color:"var(--faint)", fontSize:13 }}>total pool</span>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
      ...
      <div className="multi-grid" style={{ display:"grid", gridTemplateColumns:"1fr 340px", gap:20, alignItems:"start" }}>
        ...
        <div>
          <div style={card}>
            <h3 style={{ color:"var(--text)", fontWeight:700, fontSize:14, marginBottom:16 }}>Outcomes & Probabilities</h3>
                ...
                <div key={i} style={{ marginBottom:12 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      <span style={{ color:isWinner?"var(--warn)":"white", fontWeight:isWinner?700:500, fontSize:14 }}>
                        {outcome} {isWinner && "🏆"}
                      </span>
                    </div>
                    <div style={{ textAlign:"right" as const }}>
                      <span style={{ color, fontWeight:700, fontFamily:"var(--font-mono)", fontSize:14 }}>{pct}%</span>
                      <span style={{ color:"var(--faint)", fontSize:12, marginLeft:8 }}>{formatETH(pool)}</span>
                    </div>
                  </div>
          ...
          {/* Pool breakdown */}
          <div style={card}>
            <h3 style={{ color:"var(--text)", fontWeight:700, fontSize:14, marginBottom:14 }}>Pool Breakdown</h3>
            {market.outcomes.map((outcome, i) => {
              const pool = market.pools[i] ?? BigInt(0);
              const color = COLORS[i % COLORS.length];
              return (
                <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"5px 0", borderBottom:i<market.outcomes.length-1?"1px solid rgba(var(--fg-rgb),0.04)":"none" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ width:8, height:8, borderRadius:"50%", background:color }}/>
                    <span style={{ color:"var(--muted)", fontSize:13 }}>{outcome}</span>
                  </div>
                  <span style={{ color, fontFamily:"var(--font-mono)", fontSize:13 }}>{formatETH(pool)}</span>
                </div>
              );
            })}
            <div style={{ display:"flex", justifyContent:"space-between", borderTop:"1px solid rgba(var(--fg-rgb),0.08)", paddingTop:8, marginTop:8 }}>
              <span style={{ color:"var(--muted)", fontSize:13, fontWeight:600 }}>Total Pool</span>
              <span style={{ color:"var(--accent)", fontFamily:"var(--font-mono)", fontWeight:700 }}>{formatETH(totalPool)}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ position:"sticky", top:80 }}>
          ...
          {isOpen && (
            <div style={{ ...card, marginBottom:16 }}>
              ...
              {!isConnected ? (
                <p style={{ color:"var(--faint)", fontSize:14, textAlign:"center" }}>Connect wallet to trade</p>
              ) : success ? (
                <div style={{ textAlign:"center", padding:16 }}>
                  <div style={{ fontSize:36, marginBottom:8 }}>✅</div>
                  <p style={{ color:"var(--up)", fontWeight:700 }}>Trade placed!</p>
                </div>
              ) : (
                <>
                  <p style={{ color:"var(--faint)", fontSize:12, marginBottom:10 }}>Select outcome:</p>
                    ...
                    {market.outcomes.map((outcome, i) => {
                      ...
                      return (
                        <button key={i} className="outcome-btn" onClick={()=>setSelected(i)}
                          style={{ padding:"10px 14px", borderRadius:10, border:selected===i?`2px solid ${color}`:"1px solid rgba(var(--fg-rgb),0.08)", background:selected===i?`${color}18`:"rgba(var(--fg-rgb),0.03)", cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center", transition:"all 0.2s" }}>
                          <span style={{ color:selected===i?color:"var(--text)", fontWeight:600, fontSize:13 }}>{outcome}</span>
                          <span style={{ color:"var(--faint)", fontSize:12, fontFamily:"var(--font-mono)" }}>{pct}%</span>
                        </button>
                      );
                    })}
                  </div>
                  ...
                  <div style={{ marginBottom:12 }}>
                    ...
                    <div style={{ display:"flex", gap:6, marginTop:8 }}>
                      {["0.01","0.05","0.1","0.5"].map(q=>(
                        <button key={q} onClick={()=>setAmount(q)} style={{ flex:1, padding:"5px 0", borderRadius:8, fontSize:11, fontFamily:"var(--font-mono)", cursor:"pointer", border:amount===q?"1px solid rgba(var(--accent-rgb),0.4)":"1px solid rgba(var(--fg-rgb),0.06)", background:amount===q?"rgba(var(--accent-rgb),0.1)":"transparent", color:amount===q?"var(--accent)":"var(--faint)" }}>{q}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <button onClick={handleBuy} disabled={isPending||isConfirming||selected===null||!amount}
                    style={{ width:"100%", padding:13, borderRadius:12, fontWeight:700, fontSize:15, cursor:isPending||isConfirming||selected===null||!amount?"not-allowed":"pointer", opacity:selected===null||!amount?0.5:1, border:"none",
                      background:selected!==null?`linear-gradient(135deg,${COLORS[selected%COLORS.length]},${COLORS[(selected+1)%COLORS.length]})`:"rgba(var(--fg-rgb),0.1)", color:"var(--on-accent)",
          ...
          {expired && !market.resolved && (
            <div style={{ ...card, background:"rgba(var(--warn-rgb),0.08)", border:"1px solid rgba(var(--warn-rgb),0.25)" }}>
              <p style={{ color:"var(--warn)", fontWeight:700, marginBottom:8 }}>🕐 Ready to Resolve</p>
              <p style={{ color:"var(--muted)", fontSize:13, marginBottom:12 }}>Market expired. Oracle must be set before resolving.</p>
          ...
          {market.resolved && market.expectedPayout > BigInt(0) && !market.hasClaimed && (
            <div style={{ ...card, background:"rgba(var(--up-rgb),0.08)", border:"1px solid rgba(var(--up-rgb),0.25)" }}>
              <p style={{ color:"var(--text)", fontWeight:700, marginBottom:8 }}>🏆 You Won!</p>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:12, padding:"10px 14px", background:"rgba(0,0,0,0.2)", borderRadius:10 }}>
                <span style={{ color:"var(--muted)" }}>Your payout</span>
                <span style={{ color:"var(--up)", fontFamily:"var(--font-mono)", fontWeight:700 }}>{formatETH(market.expectedPayout)}</span>
              </div>
              {claimSuccess ? (
